...
/**
 * Génération des infos des sections de manière à etre conforme pour son utilisation dans la fonction 
 * process_section_modular()
 * 
 * @param int $courseid course id
 * @return stdclass tree about all sections infos 
 */
function build_modular_tree_sections($courseid) {
    global $DB;
    ...
    if ($sections = $DB->get_records('course_sections', array('course' => $courseid), 'section ASC')) {
        ...
        foreach($sections as $sectionid => $section) {
            ...
            if ($section->section > 0){
                ...
                $section_node->numsection = $section->section;
                $section_node->id = $section->id;
                $section_node->children = [];
                $section_node->name = $section->name;
                $section_node->visible = $section->visible;
                $section_node->hasContent = $sectioninfo->hasContent;
                $section_node->type = $sectioninfo->type;
                $section_node->parentid = $sectioninfo->parentid;
                if ($sectioninfo->parentid > 0){
                    $section_node->parentid = $mapids['num-'.$sectioninfo->parentid];
                    if (!isset($childs[$section_node->parentid])){
                        $childs[$section_node->parentid] = [];
                    }
                    $childs[$section_node->parentid][] = $section_node->id;
                }
                $parents[$section_node->id] = $mapids['num-'.$sectioninfo->parentid];
                $section_node->hasNavigation = $sectioninfo->hasNavigation;
                
                // build string with all parents 
                $listparents = '[0]';
                find_parent_recur($section_node->id, $parents, $listparents);
                $section_node->parents = $listparents;

                $section_nodes[] = $section_node;
                $section_ids[] = $section->id;
                $mapids['num-'.$section->section] = $section->id;
            }
        }
    };

    $tree_sections = new stdClass();
    $tree_sections->nodes = $section_nodes;
    $tree_sections->ids = $section_ids;
    $tree_sections->childs = $childs;
    $tree_sections->parents = $parents;

    return $tree_sections;
}

/**
 * Fonction récursive pour lister tous les parents d'une section jusqu'à la racine
 * 
 * @param int $sid section id
 * @param array $parents infos about all parent sections of a course
 * @param string $str list all parents of a section id
 */
function find_parent_recur($sid, $parents, &$str) {

	if (isset($parents[$sid]) && $parents[$sid] > 0) {
        $str .= '['.$parents[$sid].']';
        $nextsid = $parents[$sid];
	    find_parent_recur($nextsid, $parents, $str);
	}
}

/**
 * Add a new section from sectiontodup to the tree tree_sections
 * 
 * @param stdclass $sectiontodup section to duplicate
 * @param stdclass $tree_sections tree with all sections infos
 * @return stdclass the new tree with new section 
 */
function duplicate_modular_section_in_tree($sectiontodup, $tree_sections) {

    $nodes_withnewsection = [];
    $parenttopaste = -1;
    $newsection = new stdClass();
    $everpasted = false;
    foreach($tree_sections->nodes as $section_node) {

        //echo 'parenttopaste='.$parenttopaste.' et section_node->parentid='.$section_node->parentid."\n";
        if ($parenttopaste >= 0 && $parenttopaste === $section_node->parentid && !$everpasted) {
            $nodes_withnewsection[] = $newsection;
            $everpasted = true;
        }
        $nodes_withnewsection[] = $section_node;
        if ($parenttopaste >= 0 && (strpos($section_node->parents, '['.$parenttopaste.']') !== false) && !$everpasted) {
            $nodes_withnewsection[] = $newsection;
            $everpasted = true;
        }

        if ($section_node->numsection === $sectiontodup->section) {
            
            $newsection->id = -1;
            $newsection->children = [];
            $newsection->name = $section_node->name. ' (copie)';
            $newsection->visible = $section_node->visible;
            $newsection->hasContent = $section_node->hasContent;
            $newsection->type = $section_node->type;
            $newsection->parentid = $section_node->parentid;
            $newsection->hasNavigation = $section_node->hasNavigation;
            
            if (!isset($tree_sections->childs[$section_node->id])) { // s'il n'a aucun enfant
                $nodes_withnewsection[] = $newsection;
            }
            else {
                $parenttopaste = $section_node->parentid;
            }
        }
    }
    $tree_sections->nodes = $nodes_withnewsection;
    $tree_sections->newsection = $newsection;
    return $tree_sections;
}